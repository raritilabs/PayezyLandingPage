...
import WaitingForStoringEmail from "./WaitingForStoringEmail";
...
const JoinWaitListEmailFetching = ({ setModalIsOpen }) => {
  ...
  const [onClickSubmit, setOnClickSubmit] = useState(null);
  useEffect(() => {
    AOS.init({});
  }, []);
  ...
  if (!onClickSubmit) {
    return (
      <div>
        ...
        {emailStoredSuccesfully && !onClickSubmit && (
    ...
    return onClickSubmit && <WaitingForStoringEmail />;
  }
};
